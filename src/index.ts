...
class SteamMini {
    ...
    public async getUser(steamid: string) {
        try {
            if (!steamid || !steamRegexID.test(steamid))
                throw new Error("No/Invalid Steam User ID provided");
    ...
    public async getRecentlyPlayed(steamid: string, limit: number = 1) {
        try {
            if (!steamid || !steamRegexID.test(steamid))
                throw new Error("No/Invalid Steam User ID provided");
            if (limit < 1 || limit > 5)
                throw new Error("Limit should be between 1 to 5");
            const response = await this._request(
                endpoints.recentlyPlayed,
                `key=${this.apiKey}&steamid=${steamid}&limit=${limit}`
            );
